...
TouchBoard needs Java to run. On Windows or Linux, Java 8 or greater is fine. On MacOS, TouchBoard will run properly if Java 8 is used, but can not use any later Java versions (Java 9+). This is due to a recently discovered bug on Mac in all the the later versions of the Java language. If desired, you can easily install more than one version of Java on a Mac computer.  
The Java issue on Mac is described here:  
